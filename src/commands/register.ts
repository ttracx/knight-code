...
function registerLoginCommand(): void {
  const command: CommandDef = {
    name: 'login',
    description: 'Log in to Knightcode AI',
    category: 'Auth',
    handler: async (args) => {
      try {
        const { 'api-key': apiKey, oauth } = args;
        
        console.log('Authenticating with Knightcode...');
    ...
    args: [
      {
        name: 'api-key',
        description: 'API key for Knightcode AI',
        type: ArgType.STRING,
        shortFlag: 'k'
      },
      {
...
function registerAskCommand(): void {
  const command: CommandDef = {
    name: 'ask',
    description: 'Ask Knightcode a question about code or programming',
    category: 'Assistance',
    handler: async (args) => {
      try {
        const { question } = args;
        ...
        if (!isNonEmptyString(question)) {
          console.error('Please provide a question to ask Knightcode.');
          return;
        }
        
        console.log('Asking Knightcode...\n');
        ...
        console.error('Error asking Knightcode:', formatErrorForDisplay(error));
      }
    },
    args: [
      {
        name: 'question',
        description: 'Question to ask Knightcode',
        type: ArgType.STRING,
        position: 0,
        required: true
      },
      ...
      {
        name: 'model',
        description: 'Specific Knightcode model to use',
        type: ArgType.STRING,
        shortFlag: 'm',
...
function registerBugCommand(): void {
  ...
  const command = {
    name: 'bug',
    description: 'Report a bug or issue with Knightcode',
    category: 'system',
...
function registerFeedbackCommand(): void {
  ...
  const command = {
    name: 'feedback',
    description: 'Provide general feedback about Knightcode',
    category: 'system',
    async handler(args: Record<string, any>): Promise<void> {
      ...
      if (!isNonEmptyString(content)) {
        throw createUserError('Feedback content is required', {
          category: ErrorCategory.VALIDATION,
          resolution: 'Please provide your feedback about Knightcode'
        });
      }
    ...
    args: [
      {
        name: 'content',
        description: 'Your feedback about Knightcode',
        type: ArgType.STRING,
        required: true
      }
    ],
...
function registerExitCommand(): void {
  ...
  const command = {
    ...
    async handler(): Promise<void> {
      ...
      console.log('Exiting Knightcode CLI...');
      process.exit(0);
    },
    examples: [
      'exit'
...
function registerResetCommand(): void {
  ...
  const command = {
    name: 'reset',
    description: 'Reset the conversation context with Knightcode',
    category: 'session',
    async handler(): Promise<void> {
